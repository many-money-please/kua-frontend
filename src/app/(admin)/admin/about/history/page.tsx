"use client";

import { useState, useMemo } from "react";
import { useRouter } from "next/navigation";
import Link from "next/link";
import { DataTable, type Column } from "@/shared/ui/DataTable";
import { Pagination } from "@/shared/ui/Pagination";
import { ManagerSearchBar } from "@/shared/ui/SearchBar/ManagerSearchBar";
import { FiTrash2, FiMenu } from "react-icons/fi";

type HistoryItem = {
    id: string;
    year: number;
    month: number;
    content: string;
    groupId?: string;
};

// 임시 데이터
const mockData: HistoryItem[] = [
    {
        id: "1",
        year: 2025,
        month: 5,
        content: "카세계수중연맹(CMAS) 이사 재선(강철식 회장)",
        groupId: "2021-current",
    },
    {
        id: "2",
        year: 2025,
        month: 1,
        content: "강철식 제15대 회장 취임",
        groupId: "2021-current",
    },
    {
        id: "3",
        year: 2023,
        month: 1,
        content: "제24회 세계핀수영선수권대회 2026년도 국내 유치 확정",
        groupId: "2021-current",
    },
];

type YearGroup = {
    id: string;
    label: string;
    range: string;
};

const yearGroups: YearGroup[] = [
    { id: "2021-current", label: "2021~현재", range: "2021-current" },
    { id: "2011-2020", label: "2011~2020", range: "2011-2020" },
    { id: "2001-2010", label: "2001~2010", range: "2001-2010" },
    { id: "1991-2000", label: "1991~2000", range: "1991-2000" },
    { id: "1960-1990", label: "1960~1990", range: "1960-1990" },
];

export default function HistoryPage() {
    const router = useRouter();
    const [page, setPage] = useState(1);
    const [searchQuery, setSearchQuery] = useState("");
    const [searchOption, setSearchOption] = useState("내용");
    const [selectedRows, setSelectedRows] = useState<HistoryItem[]>([]);
    const [selectedGroup, setSelectedGroup] = useState<string>("2021-current");

    const filteredData = useMemo(() => {
        let data = mockData.filter((item) => item.groupId === selectedGroup);
        if (searchQuery.trim()) {
            data = data.filter((item) =>
                item.content.toLowerCase().includes(searchQuery.toLowerCase()),
            );
        }
        return data;
    }, [searchQuery, selectedGroup]);

    const paginationInfo = useMemo(() => {
        const pageSize = 10;
        const totalItems = filteredData.length;
        const totalPages = Math.max(1, Math.ceil(totalItems / pageSize));
        const currentPage = Math.min(page, totalPages);
        const startIndex = (currentPage - 1) * pageSize;
        const visibleData = filteredData.slice(
            startIndex,
            startIndex + pageSize,
        );
        return { totalPages, currentPage, visibleData, totalItems };
    }, [filteredData, page]);

    const columns: Column<HistoryItem>[] = [
        {
            key: "drag",
            header: "",
            accessor: () => (
                <div className="flex justify-center">
                    <FiMenu className="text-kua-gray400" size={20} />
                </div>
            ),
            className: "w-12",
        },
        {
            key: "year",
            header: "년도",
            accessor: (row) => (
                <span className="text-kua-gray800">{row.year}</span>
            ),
        },
        {
            key: "month",
            header: "월",
            accessor: (row) => (
                <span className="text-kua-gray800">{row.month}</span>
            ),
        },
        {
            key: "content",
            header: "내용",
            accessor: (row) => (
                <span className="text-kua-gray800 text-left">
                    {row.content}
                </span>
            ),
            className: "text-left",
        },
        {
            key: "delete",
            header: "삭제",
            accessor: (row) => (
                <div className="flex justify-center">
                    <button
                        type="button"
                        onClick={(e) => {
                            e.stopPropagation();
                            // TODO: 삭제 로직
                        }}
                        className="text-kua-gray400 hover:text-kua-orange500"
                    >
                        <FiTrash2 size={20} />
                    </button>
                </div>
            ),
        },
    ];

    const handleSearch = () => {
        setPage(1);
    };

    const handleAdd = () => {
        router.push(`/admin/about/history/create?group=${selectedGroup}`);
    };

    const handleDelete = () => {
        // TODO: 선택된 항목 삭제 로직
        console.log("삭제할 항목:", selectedRows);
    };

    const handleCreateGroup = () => {
        // TODO: 새 그룹 만들기 로직
        alert("새 그룹 만들기 기능은 준비 중입니다.");
    };

    return (
        <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
                <h1 className="text-2xl font-bold">협회 소개 관리</h1>
            </div>

            <div className="border-kua-gray300 flex gap-6 border-b">
                <Link
                    href="/admin/about/greetings"
                    className="text-kua-gray500 hover:text-kua-gray800 border-b-2 border-transparent px-4 pb-3 text-lg font-medium"
                >
                    인사말
                </Link>
                <Link
                    href="/admin/about/history"
                    className="border-kua-main text-kua-main border-b-2 px-4 pb-3 text-lg font-medium"
                >
                    연혁
                </Link>
                <Link
                    href="/admin/about/organization"
                    className="text-kua-gray500 hover:text-kua-gray800 border-b-2 border-transparent px-4 pb-3 text-lg font-medium"
                >
                    조직도
                </Link>
                <Link
                    href="/admin/about/committees"
                    className="text-kua-gray500 hover:text-kua-gray800 border-b-2 border-transparent px-4 pb-3 text-lg font-medium"
                >
                    위원회 소개
                </Link>
                <Link
                    href="/admin/about/branches"
                    className="text-kua-gray500 hover:text-kua-gray800 border-b-2 border-transparent px-4 pb-3 text-lg font-medium"
                >
                    시/도 지부소개
                </Link>
            </div>

            <div className="flex gap-6">
                {/* 왼쪽 연도 필터 */}
                <div className="border-kua-gray200 flex w-48 flex-col gap-2 rounded-lg border bg-white p-4">
                    {yearGroups.map((group) => (
                        <button
                            key={group.id}
                            type="button"
                            onClick={() => {
                                setSelectedGroup(group.id);
                                setPage(1);
                            }}
                            className={`rounded-lg px-4 py-2 text-left transition-colors ${
                                selectedGroup === group.id
                                    ? "bg-kua-main text-kua-white"
                                    : "bg-kua-gray100 text-kua-gray800 hover:bg-kua-gray200"
                            }`}
                        >
                            {group.label}
                        </button>
                    ))}
                    <button
                        type="button"
                        onClick={handleCreateGroup}
                        className="bg-kua-main text-kua-white hover:bg-kua-blue600 mt-2 rounded-lg px-4 py-2 text-sm transition-colors"
                    >
                        새 그룹 만들기
                    </button>
                </div>

                {/* 오른쪽 메인 콘텐츠 */}
                <div className="flex flex-1 flex-col gap-6">
                    <ManagerSearchBar
                        searchQuery={searchQuery}
                        searchOption={searchOption}
                        searchOptions={["내용"]}
                        onSearchQueryChange={setSearchQuery}
                        onSearchOptionChange={setSearchOption}
                        onSearch={handleSearch}
                        onAdd={handleAdd}
                        onDelete={handleDelete}
                        selectedCount={selectedRows.length}
                        placeholder="검색"
                        addButtonText="신규 생성"
                    />

                    <DataTable
                        columns={columns}
                        data={paginationInfo.visibleData}
                        getRowId={(row) => row.id}
                        onSelectionChange={setSelectedRows}
                    />

                    {paginationInfo.totalPages > 1 && (
                        <Pagination
                            currentPage={paginationInfo.currentPage}
                            totalPages={paginationInfo.totalPages}
                            onPageChange={setPage}
                        />
                    )}
                </div>
            </div>
        </div>
    );
}
